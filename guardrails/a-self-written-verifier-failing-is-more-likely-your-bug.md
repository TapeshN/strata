...
date: 2026-06-16
category: guardrails
tags: [verify-the-witness, self-written-tool, coordinator-not-exempt]
confidence: learned
source: private-work
...
Two failure modes combine: first, the coordinator applies the verification requirement to workers but suspends it for their own tooling, treating a freshly written audit script as authoritative. Second, the verifier-has-its-own-bug trap — a script that reads from the wrong data key, uses the wrong endpoint, or makes a faulty assumption about format will produce a confidently wrong result.
